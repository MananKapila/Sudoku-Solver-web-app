...
function isPossible(board, sr, sc, val) {
    ...
    //subgrid (same number should also not be present in that sub grid)
    var r = sr - sr % 3;
    var c = sc - sc % 3;
...
function solveSudokuHelper(board, sr, sc) {
    ...
    //there is 0 (means blank) in the current location
    for (var i = 1; i <= 9; i++) {
        if (isPossible(board, sr, sc, i)) {
            board[sr][sc] = i;
            var success=solveSudokuHelper(board, sr, sc + 1);
            if(success==true){
                return true;
            }
            //backtracking
